...
class AuthsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "returns an api token on successful auth" do
    user = users(:one)
    post api_v1_auth_url, params: {email: user.email, password: "P@ssw0rd64646464"}
  ...
  test "returns unauthorized if OTP attempt is required but incorrect" do
    ...
    user.enable_two_factor!
    user.set_otp_secret!
    post api_v1_auth_url, params: {email: user.email, password: "P@ssw0rd64646464", otp_attempt: "123456"}
    assert_response :unauthorized
  end
  ...
  test "returns an api token on successful auth with otp attempt" do
    ...
    user.enable_two_factor!
    user.set_otp_secret!
    post api_v1_auth_url, params: {email: user.email, password: "P@ssw0rd64646464", otp_attempt: user.current_otp}
    assert_response :success
    assert_not_nil response.parsed_body["token"]
  end
  ...
  test "sets auth cookie during hotwire app login" do
    user = users(:one)
    post api_v1_auth_url, params: {email: user.email, password: "P@ssw0rd64646464"}, headers: {HTTP_USER_AGENT: "Turbo Native iOS"}
    assert_response :success
