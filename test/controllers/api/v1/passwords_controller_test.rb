...
class PasswordsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "changes password on success" do
    user = users(:one)
    patch api_v1_password_url, params: {user: {current_password: "P@ssw0rd64646464", password: "new_password", password_confirmation: "new_password"}}, headers: {Authorization: "token #{user.api_tokens.first.token}"}
  ...
  test "errors if password confirmation doesn't match" do
    user = users(:one)
    patch api_v1_password_url, params: {user: {current_password: "P@ssw0rd64646464", password: "new_password", password_confirmation: "wrong_password"}}, headers: {Authorization: "token #{user.api_tokens.first.token}"}
    assert_response :unprocessable_entity
    assert_not_nil json_response.dig("error")
  end
end
